...
const session = { // 사용자 마다 유니크 인트가 생성됨 유니크인트는 오브젝트 형태로 유저인포가 들어감
}
...
http.createServer(async(req,res)=>{
    ...
    if (req.url.startsWith('/login')) { // req.url >> /login 으로 시작하는지 확인
        ...
        res.writeHead(302, {
            ...
            'Set-Cookie' : `session=${uniqueInt}; Expires=${expires.toGMTString()}; HttpOnly; Path=/`, // 쿠키를 생성 하고 헤더 정보에 들어감   
        }); // 쿠키,url로 변환하기 위해 아스키 코드로 변환시킴  // HttpOnly << Http로만 쿠키에 접근을 하도록 함 //Path >> 쿠키가 가지고 있는 path 정보
        res.end();
    } else if (cookies.session && session[cookies.session].expires > new Date()) { // 아직 유효시간이 남아잇으면
